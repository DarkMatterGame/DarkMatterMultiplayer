//*********************************************************//
//DARK MATTER MULTIPLAYER CLIENT COMPONENT
//*********//
//LIAM OSLER
//LIAMOSLER.COM
//MARCH 2019
...
//Map Scale (mapScale) variable (controlled by mousewheel):
var mapScale = 1;
//Mousewheelup/Mousewheeldown, increase decrease mapScale:
...
//Call onKeyUp/onKeyDown functions:
...
//ListLog functions (for retrieving data and storing locally via socket
function clientListLog(data){playerVarServer = data;}

function asteroidListLog(data){clientAsteroids = data;}
...
function drawCursor(){
	...
	if(mouseDown == true){
		for(i = 0; i<clientAsteroids.id.length; i++){
			if (inverse(clientAsteroids.dispX[i],clientAsteroids.dispY[i],mouseX,mouseY)<50){
				clientAsteroids.stat[i]=1;
				}
			else{
				clientRadar.harvester=-1;
				clientRadar.filler=-1;
				clientAsteroids.stat[i]=-1;
				}
			}	
		}	
	...
	if(playerVarLocal.laserStatus==true){
		c.strokeStyle = "#FF000";
	}
...
function drawMinimap(){
	...
	for (var i = 0; i<clientAsteroids.id.length; i++){
		miniMapDistance = inverse((clientAsteroids.posX[i]*0.1)-(playerVarLocal.posX*0.1)+midX+w/2-300, (clientAsteroids.posY[i]*0.1)-(playerVarLocal.posY*0.1)+midY-150, miniMapX, miniMapY-150);
		if (miniMapDistance>150){}
		else{
			c.fillStyle = 'rgba(0, 0, 0, 1.000)'
			c.beginPath();
			c.arc((clientAsteroids.posX[i]*0.1)-(playerVarLocal.posX*0.1)+midX+w/2-300, (clientAsteroids.posY[i]*0.1)-(playerVarLocal.posY*0.1)+midY-150, 3, 0, Math.PI*2);
			c.closePath();
			c.fill();
		}
	}		
	...
	/* 	for (var i = 0; i<star.id.length; i++){
			miniMapDistance = inverse((star.posX[i]*0.1)-(playerVarLocal.posX*0.1)+midX+w/2-300, (star.posY[i]*0.1)-(playerVarLocal.posY*0.1)+midY-150, miniMapX, miniMapY-150);
			miniMapDir = azCalc((star.posX[i]*0.1)-(playerVarLocal.posX*0.1)+midX+w/2-300, (star.posY[i]*0.1)-(playerVarLocal.posY*0.1)+midY-150, miniMapX, miniMapY-150);
			...
			if (miniMapDistance+star.radius[i]*0.05>200){
				...
				c.beginPath();
				c.arc((star.posX[i]*0.1)-(playerVarLocal.posX*0.1)+midX+w/2-300, (star.posY[i]*0.1)-(playerVarLocal.posY*0.1)+midY-150,star.radius[i]*0.05 , 0, Math.PI*2);
				c.closePath();
				c.fill();		
				}
			...
			if (miniMapDistance+star.radius[i]*0.05<200){
				...
				c.lineTo(  ((star.posX[i]*0.1)-(playerVarLocal.posX*0.1)+midX+w/2-300)    ,((star.posY[i]*0.1)-(playerVarLocal.posY*0.1)+midY-150))  ;
				c.closePath();
				c.stroke();
				} 
...
//Client radar screen draw (originates from center of screen):
function drawclientRadar(){
	if (clientRadar.active == 1){
		if(blackOutStatus==false && whiteOutStatus==false && redOutStatus==false){
			//clientRadarSound.play();
		}
	for(var i = 0; i<clientAsteroids.id.length; i++){
		var perpAng = azCalc(midX, midY,clientAsteroids.dispX[i], clientAsteroids.dispY[i]);
		var distclientRadar = inverse(midX, midY, clientAsteroids.dispX[i], clientAsteroids.dispY[i]);
		var dispDistclientRadar = 2000*mapScale;
		var coneX1, coneY1, coneX2, coneY2, coneAz1, coneAz2;
		var drawConeX1, drawConeY1, drawConeX2, drawConeY2; 
		
		coneX1= clientAsteroids.dispX[i]+lats(perpAng,clientAsteroids.radius[i]*mapScale);
		coneY1 = clientAsteroids.dispY[i]+deps(perpAng,clientAsteroids.radius[i]*mapScale);
		coneX2= clientAsteroids.dispX[i]-lats(perpAng,clientAsteroids.radius[i]*mapScale);
		coneY2 = clientAsteroids.dispY[i]-deps(perpAng,clientAsteroids.radius[i]*mapScale);
		coneAz1 = azCalc(midX, midY, coneX1, coneY1);
		coneAz2 = azCalc(midX, midY, coneX2, coneY2);
		
		drawConeX1 = midX-lats(coneAz1+90-10,dispDistclientRadar);
		drawConeY1 = midY-deps(coneAz1+90-10,dispDistclientRadar);
		drawConeX2 = midX-lats(coneAz2+90+10,dispDistclientRadar);
		drawConeY2 = midY-deps(coneAz2+90+10,dispDistclientRadar);
		
		// Create gradient
		grd = c.createRadialGradient((drawConeX1+drawConeX2)/2,(drawConeY1+drawConeY2)/2,0,midX, midY,dispDistclientRadar); 
		// Add colors
		...
		if(distclientRadar<2000*mapScale){
				c.beginPath();
				c.moveTo(midX, midY);
			...
			if(distclientRadar<200*mapScale){
		c.beginPath();
		c.moveTo(midX, midY);
		
		c.lineTo(clientAsteroids.dispX[i]+lats(perpAng+t*200,clientAsteroids.radius[i])*mapScale, clientAsteroids.dispY[i]+deps(perpAng+t*200,clientAsteroids.radius[i])*mapScale);
		c.closePath();
		c.strokeStyle="#00FF00";
		c.stroke();
			}
		...
		//clientRadarSound.stop();
	}	
}
//Local player sprite draw:
function playerSprite(){
	...
	c.lineWidth=5;
	c.beginPath();
	c.moveTo(midX-lats(playerVarLocal.az+90,20 *mapScale), midY-deps(playerVarLocal.az+90,20  *mapScale));
	c.lineTo(midX+lats(playerVarLocal.az+90,20 *mapScale), midY+deps(playerVarLocal.az+90,20  *mapScale));
	c.closePath();
	c.stroke();
	c.lineWidth=3;
	c.beginPath();
	c.moveTo(midX-lats(playerVarLocal.az-45,20 *mapScale), midY-deps(playerVarLocal.az-45,20 *mapScale));
	c.lineTo(midX+lats(playerVarLocal.az+90,20 *mapScale ), midY+deps(playerVarLocal.az+90,20*mapScale ));
	c.moveTo(midX+lats(playerVarLocal.az+45,20  *mapScale), midY+deps(playerVarLocal.az+45,20*mapScale ));
	c.lineTo(midX-lats(playerVarLocal.az-90,20 *mapScale ), midY-deps(playerVarLocal.az-90,20*mapScale ));
	c.moveTo(midX-lats(playerVarLocal.az+270,5 *mapScale), midY-deps(playerVarLocal.az+270,5  *mapScale));
	c.lineTo(midX+lats(playerVarLocal.az+60,20	 *mapScale), midY+deps(playerVarLocal.az+60,20  *mapScale));
	c.moveTo(midX+lats(playerVarLocal.az-270,5 *mapScale), midY+deps(playerVarLocal.az-270,5  *mapScale));
	c.lineTo(midX-lats(playerVarLocal.az-60,20	 *mapScale), midY-deps(playerVarLocal.az-60,20  *mapScale));
	c.closePath();
	c.stroke();
	...
	if(playerVarLocal.thrust>1){		
		grdThrust = c.createRadialGradient(midX-lats(playerVarLocal.az+90,-20 *mapScale), midY-deps(playerVarLocal.az+90,-20  *mapScale),0, midX-lats(playerVarLocal.az+90,-20 *mapScale), midY-deps(playerVarLocal.az+90,-20  *mapScale), 20);
		...
		c.beginPath();
		c.arc(midX-lats(playerVarLocal.az+90,-28 *mapScale), midY-deps(playerVarLocal.az+90,-28  *mapScale), 10, 0, Math.PI*2);
		c.closePath();
		c.fill();
		c.beginPath();
		c.arc(midX-lats(playerVarLocal.az+115,-30 *mapScale), midY-deps(playerVarLocal.az+115,-30  *mapScale), 10, 0, Math.PI*2);
		c.closePath();
		c.fill();
		c.beginPath();
		c.arc(midX-lats(playerVarLocal.az+65,-30 *mapScale), midY-deps(playerVarLocal.az+65,-30  *mapScale), 10, 0, Math.PI*2);
		c.closePath();
		c.fill();
		
		if(shiftDown == true){
			grdThrust = c.createRadialGradient(midX-lats(playerVarLocal.az+90,-40 *mapScale), midY-deps(playerVarLocal.az+90,-30  *mapScale),0, midX-lats(playerVarLocal.az+90,-40 *mapScale), midY-deps(playerVarLocal.az+90,-40  *mapScale), 20);
			grdThrust.addColorStop(1.000, 'rgba(000, 000, 000, 0.000)');
			grdThrust.addColorStop(0.500, 'rgba(255, 255, 255, 1.000)');
			grdThrust.addColorStop(0.000, 'rgba(255, 255, 255, 1.000)');
			c.beginPath();
			c.arc(midX-lats(playerVarLocal.az+90,40*mapScale), midY-deps(playerVarLocal.az+90,40 *mapScale),70, 0, Math.PI*2);
			c.closePath();
			c.fill();
...
//Test drawing function:(from tutorial) on mouseDown:
...
function drawclientAsteroids(){
	var grd;
	for (i = 0; i<clientAsteroids.id.length; i++){
		var distVisib = inverse(midX, midY, clientAsteroids.dispX[i], clientAsteroids.dispY[i]);
		
		if (distVisib<1500){
			c.beginPath();
			c.arc(clientAsteroids.dispX[i], clientAsteroids.dispY[i], ((clientAsteroids.radius[i]*1.5)*mapScale), 0, Math.PI*2);
			c.closePath();
			grdAst = c.createRadialGradient(clientAsteroids.dispX[i], clientAsteroids.dispY[i], 0.000, clientAsteroids.dispX[i], clientAsteroids.dispY[i], (clientAsteroids.radius[i]*1.5)*mapScale);
			...
			c.arc(clientAsteroids.dispX[i], clientAsteroids.dispY[i], (clientAsteroids.radius[i]*mapScale), 0, Math.PI*2);
			c.closePath();
			c.fill();
			c.stroke();
	
				
		//If clientAsteroids is selected:
		if (clientAsteroids.stat[i] == 1){
			c.strokeStyle= "#00FF00";
			if(clientRadar.harvester==1){
				c.strokeStyle= "#00FFFF";
			}
			if(clientRadar.filler==1){
				c.strokeStyle= "#FFFF00";
			}
			c.beginPath();
			c.arc(clientAsteroids.dispX[i], clientAsteroids.dispY[i],(((Math.abs(Math.sin(t))*10)*mapScale)+clientAsteroids.radius[i]+10)*mapScale, 0, Math.PI*2);
			c.closePath();
			c.stroke();
		}
		...
		c.font = "15px Arial";
		c.fillStyle="#FF0000";
		c.fillText('id: ' + clientAsteroids.id[i] , clientAsteroids.dispX[i]-40, clientAsteroids.dispY[i]-30);
		c.fillText('PosX: ' + Math.floor(clientAsteroids.posX[i]) , clientAsteroids.dispX[i]-40, clientAsteroids.dispY[i]-15);
		c.fillText('PosY: ' + Math.floor(clientAsteroids.posY[i]) , clientAsteroids.dispX[i]-40, clientAsteroids.dispY[i]);	
		c.fillText('velX: ' + Math.floor(clientAsteroids.velX[i]) , clientAsteroids.dispX[i]-40, clientAsteroids.dispY[i]+15);
		c.fillText('velY: ' + Math.floor(clientAsteroids.velY[i]) , clientAsteroids.dispX[i]-40, clientAsteroids.dispY[i]+30);
		c.fillText('mass: ' + Math.floor(clientAsteroids.mass[i]) , clientAsteroids.dispX[i]-40, clientAsteroids.dispY[i]+60);
		c.fillText('ΣfX: ' + Math.floor(clientAsteroids.fX[i]), clientAsteroids.dispX[i]-40, clientAsteroids.dispY[i]+75);
		c.fillText('ΣfY: ' + Math.floor(clientAsteroids.fY[i]), clientAsteroids.dispX[i]-40, clientAsteroids.dispY[i]+90);
		}
		
		for(j=0; j<clientAsteroids.id.length[i];j++){
			c.beginPath();
			c.fillStyle = "#000";
			c.arc(clientAsteroids.dispX[j], clientAsteroids.dispY[j], (clientAsteroids.radius[j]*mapScale), 0, Math.PI*2);
			c.closePath();
			c.fill();
		}
}
//Translate absolute coordinates to screen coordiantes for clientAsteroids:
function clientAsteroidsDisplayPositionHelper(){
	for (i = 0; i<clientAsteroids.id.length; i++){
		clientAsteroids.dispX[i] = (clientAsteroids.posX[i]*mapScale)-(playerVarLocal.posX*mapScale)+midX;
		clientAsteroids.dispY[i] = (clientAsteroids.posY[i]*mapScale)-(playerVarLocal.posY*mapScale)+midY;
		}
}

function clientPlayersDisplayPositionHelper(){
	for (i = 0; i<clientPlayers.id.length; i++){
		clientPlayers.dispX[i] = (clientPlayers.posX[i]*mapScale)-(playerVarLocal.posX*mapScale)+midX;
		clientPlayers.dispY[i] = (clientPlayers.posY[i]*mapScale)-(playerVarLocal.posY*mapScale)+midY;
		}
	}
...
function drawMiniStars(){
	for (var i = 0; i<starCoords.posX.length; i++){
		c.beginPath();
		c.arc((starCoords.posX[i]*30)-(playerVarLocal.posX/10), (starCoords.posY[i]*30)-(playerVarLocal.posY/10), 1, 0, Math.PI*2);
		c.closePath();
		c.fillStyle="#FFF";
		c.fill();
	}
...
function vignetteScreen(){
	//Create gradient centered in window:
    grdVig = c.createRadialGradient(midX, midY, 0.000, midX, midY, 0.5*w*mapScale); 
    //Add colors, from black transparent to black opaque:
    grdVig.addColorStop(0.30, 'rgba(0, 0, 0, 0.000)');
    grdVig.addColorStop(1.00, 'rgba(0, 0, 0, 1.000)');
	c.fillStyle = grdVig;
	c.rect(0,0, w ,h);
	c.fill();
}
...
//client onBodyLoad(index.html) init:
function init(){
//Connect to socket:
socket= io.connect("http://localhost:3000");
//Set interval of canvas: 
c = gameCanvas.getContext('2d');	
setInterval(draw,25);
		function draw(event){
		
		socket.on('clientList', clientListLog);
		socket.on('asteroidList', asteroidListLog);
		...
		//If loadscreen is not being called draw the game:
			...
			if(spaceDown == true){
				...
				c.font = "30px Arial";
				c.fillStyle="#000";
				c.fillText('Clients: ' + clientPlayers.id, 180,50);
				c.fillRect(mouseX, mouseY, 10,10);
		
				
				//Call client draw functions:
				clientAsteroidsDisplayPositionHelper();
				drawclientAsteroids();
				playerSprite();
				//vignetteScreen();
				drawMinimap();
